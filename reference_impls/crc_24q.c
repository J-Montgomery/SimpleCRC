...
int main(int argc, char *argv[])
{
    int i;
    unsigned crc;
    /*char buf2[] = {0xD3, 0x00, 0x26, 0x3E, 0x70, 0x10, 0x0A, 0xB0, 0x22, 
                0x02, 0x12, 0x44, 0x80, 0xBF, 0xFF, 0xFF, 0xC0, 0x00,
                0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x02, 0x0A, 0x3C, 0x00, 0x00, 0x02,
                0x1F, 0x8F, 0xF1, 0x79, 0x10, 0x70, 0x3A, 0xD8};*/
    char buf2[] = {0xFF, 0xD0, 0xC0};
    char buf[] = "123456789";

    if(argc != 2) {
        printf("crc_24q <input>\n");
        crc = crc24q_hash(buf, sizeof(buf) - 1);
        printf("0x%.8x\n", crc);
        crc = crc24q_hash(buf2, sizeof(buf2));
        printf("0x%.8x\n", crc);
    } else {
        crc = crc24q_hash(argv[1], strlen(argv[1]));
        printf("0x%.8x\n", crc);
    }

    for (i = 0; i < 256; i++) {
	printf("0x%.8x, ", crc24q[i] & 0xFFFFFF);
	if ((i % 4) == 3)
	    putchar('\n');
    }
